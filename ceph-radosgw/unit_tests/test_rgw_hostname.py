...
class TestRgwHostnameHelpers(unittest.TestCase):
    ...
    def test_apache_ssl_vhosts_normalises_endpoint_before_matching(self):
        endpoints = [
            ('10.0.0.10', 'rgw.example.com, s3.example.com,', 443, 80),
        ]

        vhosts = rgw_hostname.apache_ssl_vhosts(
            endpoints,
            ' rgw.example.com,s3.example.com ',
            True,
        )
        ...
        self.assertEqual(
            ['rgw.example.com', 's3.example.com'],
            [vhost['endpoint'] for vhost in vhosts],
        )
    ...
    def test_certificate_requests(self):
        ...
        original_requests = copy.deepcopy(cert_requests)
        ...
        self.assertEqual(original_requests, cert_requests)

    def test_certificate_requests_normalises_key_before_matching(self):
        cert_requests = {
            'rgw.example.com, s3.example.com,': {
                'sans': [
                    '10.0.0.10',
        ...
        requests = rgw_hostname.certificate_requests(
            cert_requests,
            ' rgw.example.com,s3.example.com ',
            virtual_hosted_bucket_enabled=True,
        )
