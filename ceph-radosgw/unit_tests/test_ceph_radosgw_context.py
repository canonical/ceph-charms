...
class ApacheContextTest(CharmTestCase):
    ...
    def test_configure_cert_expands_public_hostnames(
        ...
        self.test_config.set(
            'os-public-hostname', 'rgw.example.com, s3.example.com,')
        ssl_context = context.ApacheSSLContext()

        ssl_context.configure_cert('rgw.example.com,s3.example.com')

        self.assertEqual(
            [call('rgw.example.com'), call('s3.example.com')],
            mock_configure_cert.call_args_list,
        )

    @patch('ceph_radosgw_context.context.ApacheSSLContext.configure_cert')
    def test_configure_cert_preserves_other_cns(self, mock_configure_cert):
        self.test_config.set(
            'os-public-hostname', 'rgw.example.com,s3.example.com')

        context.ApacheSSLContext().configure_cert('10.0.0.10')
    ...
    def test_certificate_relation_takes_precedence_over_static_cert(self):
        ...
        with patch.object(context.context, 'https', return_value=True), \
                patch.object(context.context, 'relation_ids',
                             return_value=['certificates:1']), \
                patch.object(context.context, 'related_units',
                             return_value=['vault/0']), \
                patch.object(base_context, 'enable_modules'), \
                patch.object(base_context, 'get_network_addresses',
                             return_value=[(
                                 '10.0.0.10',
                                 'rgw.example.com,s3.example.com',
                             )]), \
                patch.object(context.context, 'determine_apache_port',
                             return_value=443), \
                patch.object(context.context, 'determine_api_port',
                             return_value=80), \
                patch.object(context.ApacheSSLContext,
                             'configure_cert') as mock_configure_cert:
            context.ApacheSSLContext()()

        mock_configure_cert.assert_not_called()
